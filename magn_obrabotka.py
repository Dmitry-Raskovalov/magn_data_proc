"""
На вход принимается один или несколько файлов с измерениями магнитометра минимаг
На выходе таблица excel с данными из этих файлов.
С данными проводится минимальная обработка:
 помечаются и удаляются повторы (оставляется последнее измерение)
 и помечается сбой нумерации измерений, 
 ненужные измерения удаляются вручную в Excel.
"""
...
class Source_file():
   ...
   # ключевые слова по которым определяется формат файла
    ...
    def read_header(self, line):
        ...
        забирает из него дату и формат файла
        """
        self.date = line[-9:-1]
        self.file.readline()
    ...
    def read_file(self, magn_db):
        ...
        после запускает обработку если в файле были измерения (если были только вариации обработка не запускается) """
        processing = False
...
class File_choise():
    ...
    def check_input (self, choice, is_list=False):
        ...
        Если выбран пункт меню передает флаг(is_add) на добавление  данных в файл, 
        если введено имя файла - флаг записи в новый.
